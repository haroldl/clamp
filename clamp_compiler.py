...
def codegen_funcall(node, context : Context):
    ...
    if isinstance(node.func, ast.Name) and node.func.id.lower() in {"print", "len", "bool", "callable", "repr", "str", "type", "iter", "next", "reversed", "min", "max", "sum", "sorted", "list", "tuple", "abs", "hash", "divmod", "all", "any", "enumerate", "zip", "filter", "range", "slice", "chr", "ord"}:
