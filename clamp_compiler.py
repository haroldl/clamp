...
def codegen_funcall(node, context : Context):
    ...
    if isinstance(node.func, ast.Name) and node.func.id.lower() in {"print", "len", "bool", "repr", "iter", "next", "reversed", "min", "max", "sum", "sorted", "abs", "hash", "all", "any", "enumerate", "zip", "filter"}:
